...
CCLayer* FinishOverlay::createFinish(float time, int peanuts, int points)
{
    ...
    else if(LevelSelect::getCurrentLevel() >= BONUS_LEVELSTART && LevelSelect::getCurrentLevel() <= (BONUS_LEVELSTART+3))
        return new BonusFinishOverlay(time,peanuts,points);    
    else
        return new FinishOverlay(time,peanuts,points);
}

BonusFinishOverlay::~BonusFinishOverlay()
{
    
}
BonusFinishOverlay::BonusFinishOverlay(float time, int peanuts, int points)
{
    ScreenInit(time,peanuts,points);
}
