...
void TikiBackGround::InitBG(cocos2d::CCNode *layer)
{
    ...
    if(tikiObjsLoaded)
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            sprite = CCSprite::createWithSpriteFrameName(name);
            sprite->setScale(scale);
            m_Parallax3Points[i] = ScreenHelper::getAnchorPointPlusOffset(ScreenHelper::ANCHOR_BOTTOM_LEFT, 256.0f*i+CCRANDOM_MINUS1_1()*25.0f, 63.0f+CCRANDOM_MINUS1_1()*10.0f);
