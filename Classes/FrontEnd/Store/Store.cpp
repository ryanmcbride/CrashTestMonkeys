...
Store::Store()
{
    ...
    CCNode *first = m_UpgradeButtons[0];
    CCNode *last = m_Bikebuttons[7];
    ...
    menu_size.height += 22*scaleY;
    
    CCNode *holder = CCNode::create();
    holder->setContentSize(menu_size);
    holder->addChild(screenMenu);
    screenMenu->setAnchorPoint(ccp(0.0f,0.0f));
    screenMenu->setPosition(ccp(menu_size.width/2.0f,menu_size.height/2.0f-11*scaleY));
    
    CCScrollView *scrollView = CCScrollView::create(CCSizeMake(menu_size.width+10*scaleX, 190*scaleY),holder);
    scrollView->setContentSize(menu_size);
    scrollView->updateInset();
    scrollView->setPosition(ccp((470)*scaleX - menu_size.width,30*scaleY));
    scrollView->setContentOffset(ccp(0.0f,-menu_size.height+(190*scaleY)));
