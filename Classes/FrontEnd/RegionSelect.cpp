...
RegionSelect::RegionSelect()
{
    ...
    holder->setContentSize(CCSizeMake(2320.0f*scaleX,320*scaleY));
    holder->addChild(screenMenu);
    screenMenu->setAnchorPoint(ccp(0.0f,0.0f));
    screenMenu->setPosition(ccp(2320.0f*scaleX/2.0f,100*scaleY));

    CCScrollView *scrollView = CCScrollView::create(CCSizeMake(470*scaleX, 190*scaleY),holder);
    
    scrollView->setContentSize(CCSizeMake(2320.0f*scaleX,320*scaleY));
    scrollView->updateInset();
    addChild(scrollView);
    scrollView->setDirection(kCCScrollViewDirectionHorizontal);
    scrollView->setAnchorPoint(ccp(0.5f,0.5f));
    scrollView->setPosition(ccp(5*scaleX,30*scaleY));
    
    m_Scrollview = scrollView;
    
    if(m_fScrollViewOffset > -10000.0f)
        scrollView->setContentOffset(ccp(m_fScrollViewOffset,0.0f));
}
